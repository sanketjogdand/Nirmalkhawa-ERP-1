<div class="product-container">
    ...
    <div style="display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:12px;">
        <h2 class="page-heading" style="margin-bottom:0;">Center Payment</h2>
        <div style="display:flex; gap:10px; flex-wrap:wrap;">
            <a href="{{ route('center-payments.view') }}" class="btn-primary" wire:navigate>Back to list</a>
            @can('centerpayment.update')
                <a href="{{ route('center-payments.edit', $payment->id) }}" class="btn-primary" wire:navigate>Edit Payment</a>
            @endcan
        </div>
    </div>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="px-4 py-2 border dark:border-zinc-700">
                        @if($payment->center)
                            <a href="{{ route('centers.show', $payment->center->id) }}" class="action-link" wire:navigate>{{ $payment->center->name }}</a>
                        @endif
                    </td>
                </tr>
                ...
                <tr><th class="px-4 py-2 border dark:border-zinc-700">Lock Status</th>
                    <td class="px-4 py-2 border dark:border-zinc-700">
                        {{ $payment->is_locked ? 'Locked' : 'Open' }}
                        @if($payment->is_locked)
                            <div style="font-size:12px; color:gray;">By: {{ $payment->lockedBy?->name }} @if($payment->locked_at) | {{ $payment->locked_at->format('d M Y H:i') }} @endif</div>
                        @endif
                    </td>
                </tr>
