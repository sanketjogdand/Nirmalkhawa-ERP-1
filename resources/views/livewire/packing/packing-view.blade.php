<div class="product-container">
    ...
    <div style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 12px;">
        <h2 class="page-heading" style="margin-bottom: 0;">Packing</h2>
        <div style="display:flex; gap:10px; flex-wrap:wrap;">
            @can('packing.create')
                <a href="{{ route('packings.create') }}" class="btn-primary" wire:navigate>New Packing</a>
            @endcan
        </div>
    </div>
                ...
                @forelse ($records as $record)
                    <tr>
                        <td class="px-4 py-2 border dark:border-zinc-700">
                            <input type="checkbox" value="{{ $record->id }}" wire:model.live="selected" @disabled($record->is_locked && !auth()->user()->can('packing.unlock')) onclick="event.stopPropagation();">
                        </td>
                        ...
                        <td class="px-4 py-2 border dark:border-zinc-700" style="white-space:nowrap;">
                            <span style="display:inline-flex; align-items:center; gap:8px; white-space:nowrap;">
                                <a href="{{ route('packings.show', $record->id) }}" class="action-link" wire:navigate>View</a>
                                @if(! $record->is_locked)
                                    @can('packing.update')
                                        <span aria-hidden="true">|</span>
                                        <a href="{{ route('packings.edit', $record->id) }}" class="action-link" wire:navigate>Edit</a>
                                    @endcan
                                    @can('packing.delete')
                                        <span aria-hidden="true">|</span>
