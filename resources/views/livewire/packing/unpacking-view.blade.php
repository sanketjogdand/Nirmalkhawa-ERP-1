<div class="product-container">
    ...
    <div style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 12px;">
        <h2 class="page-heading" style="margin-bottom: 0;">Unpacking</h2>
        <div style="display:flex; gap:10px; flex-wrap:wrap;">
            @can('unpacking.create')
                <a href="{{ route('unpackings.create') }}" class="btn-primary" wire:navigate>New Unpacking</a>
            @endcan
        </div>
    </div>
                ...
                @forelse ($records as $record)
                    <tr>
                        <td class="px-4 py-2 border dark:border-zinc-700">
                            <input type="checkbox" value="{{ $record->id }}" wire:model.live="selected" @disabled($record->is_locked && !auth()->user()->can('unpacking.unlock')) onclick="event.stopPropagation();">
                        </td>
                        <td class="px-4 py-2 border dark:border-zinc-700">{{ $record->date?->format('Y-m-d') }}</td>
                        <td class="px-4 py-2 border dark:border-zinc-700">
                            {{ $record->product?->name ?? 'N/A' }}
                            <div style="font-size:12px; color:gray;">{{ $record->product?->code }}</div>
                        </td>
                        <td class="px-4 py-2 border dark:border-zinc-700">{{ number_format($record->total_bulk_qty ?? 0, 3) }}</td>
                        <td class="px-4 py-2 border dark:border-zinc-700">
                            @if($record->is_locked)
                        ...
                        <td class="px-4 py-2 border dark:border-zinc-700" style="white-space:nowrap;">
                            <span style="display:inline-flex; align-items:center; gap:8px; white-space:nowrap;">
                                <a href="{{ route('unpackings.show', $record->id) }}" class="action-link" wire:navigate>View</a>
                                @if(! $record->is_locked)
                                    @can('unpacking.update')
                                        <span aria-hidden="true">|</span>
                                        <a href="{{ route('unpackings.edit', $record->id) }}" class="action-link" wire:navigate>Edit</a>
                                    @endcan
                                    @can('unpacking.delete')
                                        <span aria-hidden="true">|</span>
                                        <button type="button" class="action-link" style="border:none; background:transparent; padding:0;" wire:click="confirmDelete({{ $record->id }})">Delete</button>
                                    @endcan
                                    @can('unpacking.lock')
                                        <span aria-hidden="true">|</span>
                                        <button type="button" class="action-link" style="border:none; background:transparent; padding:0;" wire:click="confirmLock({{ $record->id }})">Lock</button>
                                    @endcan
                                @else
                                    @can('unpacking.unlock')
                                        <span aria-hidden="true">|</span>
                                        <button type="button" class="action-link" style="border:none; background:transparent; padding:0;" wire:click="confirmUnlock({{ $record->id }})">Unlock</button>
                                    @endcan
                                @endif
                            </span>
                        </td>
    ...
    @if($showUnlockModal)
        <div style="position:fixed; inset:0; background:rgba(0,0,0,0.65); display:flex; align-items:center; justify-content:center; z-index:1000;">
            <div style="background:#111827; color:#e5e7eb; padding:20px; border-radius:12px; max-width:420px; width:90%; border:1px solid #374151;">
                <h3 style="margin-top:0; font-size:18px;">Unlock selected records?</h3>
                <p style="margin:8px 0;">Unlocked records can be edited or deleted.</p>
                <div style="display:flex; gap:12px; justify-content:flex-end; margin-top:16px;">
