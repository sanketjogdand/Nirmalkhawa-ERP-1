        ...
        @else
            <div style="margin: 1rem 0;">
                <div class="summary-card">
                    <div class="summary-title">Bulk Qty Returned</div>
                    <div class="summary-value">{{ number_format($this->totalBulkReturn, 3) }} {{ $products->firstWhere('id', (int) $product_id)?->uom }}</div>
                </div>
            </div>
                ...
                <table class="product-table hover-highlight">
                    <thead>
                        <tr>
                            ...
                            <th class="px-4 py-2 border dark:border-zinc-700">Available Packs</th>
                            <th class="px-4 py-2 border dark:border-zinc-700">Unpack Packs</th>
                    ...
                    <tbody>
                        @foreach($lines as $index => $line)
                            @php
                                ...
                                $availablePacks = $line['pack_size_id'] ? ($packInventory[$line['pack_size_id']] ?? 0) : 0;
                            @endphp
                            <tr>
                                <td class="px-4 py-2 border dark:border-zinc-700">
                                    <select wire:model.live="lines.{{ $index }}.pack_size_id" class="input-field">
                                ...
                                <td class="px-4 py-2 border dark:border-zinc-700">{{ $availablePacks }}</td>
                                <td class="px-4 py-2 border dark:border-zinc-700">
                                    <input type="number" min="1" step="1" wire:model.live="lines.{{ $index }}.pack_count" class="input-field" placeholder="0">
            ...
            <div style="margin-top:12px; display:flex; gap:10px; flex-wrap:wrap;">
                <button type="button" class="btn-secondary" wire:click="addLine">Add Line</button>
                <button type="button" class="btn-submit" wire:click="save">Save Unpacking</button>
            </div>
        @endif
    @endif
