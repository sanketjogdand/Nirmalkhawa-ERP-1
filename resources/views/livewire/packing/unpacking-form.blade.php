<div class="product-container">
    ...
    <div style="display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:12px;">
        <h2 class="page-heading" style="margin-bottom:0;">
            {{ $unpackingId ? ($isReadOnly ? 'View Unpacking' : 'Edit Unpacking') : 'New Unpacking' }}
        </h2>
        <div style="display:flex; gap:10px; flex-wrap:wrap;">
            <a href="{{ route('unpackings.view') }}" class="btn-secondary" wire:navigate>Back to List</a>
        </div>
    </div>
            ...
            <div style="margin-top:12px; display:flex; gap:10px; flex-wrap:wrap;">
                ...
                @if(! $isLocked && ! $isReadOnly)
                    <button type="button" class="btn-submit" wire:click="save">
                        {{ $unpackingId ? 'Save Changes' : 'Save Unpacking' }}
                    </button>
                @endif
            </div>
        @endif
