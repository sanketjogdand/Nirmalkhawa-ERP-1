<div class="product-container">
    ...
    <div style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 12px;">
        <h2 class="page-heading" style="margin-bottom: 0;">Incentives</h2>
        <div style="display:flex; gap:10px; flex-wrap:wrap;">
            @can('incentive.create')
                <a href="{{ route('employee-incentives.create') }}" class="btn-primary" wire:navigate>Add Incentive</a>
            @endcan
        </div>
    </div>
            ...
            <tbody>
                @forelse ($records as $record)
                    <tr>
                        ...
                        <td class="px-4 py-2 border dark:border-zinc-700">{{ $record->incentive_date?->format('Y-m-d') }}</td>
                        <td class="px-4 py-2 border dark:border-zinc-700">{{ $record->employee?->name }}</td>
                        <td class="px-4 py-2 border dark:border-zinc-700">{{ $record->incentive_type ?? '—' }}</td>
                        ...
                        <td class="px-4 py-2 border dark:border-zinc-700" style="white-space:nowrap;">
                            <span style="display:inline-flex; align-items:center; gap:8px; white-space:nowrap;">
                                @if(! $record->is_locked)
                                    @can('incentive.update')
                                        <a href="{{ route('employee-incentives.edit', $record->id) }}" class="action-link" wire:navigate>Edit</a>
                                    @endcan
                                    @can('incentive.delete')
                ...
                @empty
                    <tr>
                        <td colspan="7" class="px-4 py-2 border dark:border-zinc-700" style="text-align:center;">No incentives found.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
    ...
    @if($showLockModal)
        <div style="position:fixed; inset:0; background:rgba(0,0,0,0.65); display:flex; align-items:center; justify-content:center; z-index:1000;">
            <div style="background:#111827; color:#e5e7eb; padding:20px; border-radius:12px; max-width:420px; width:90%; border:1px solid #374151;">
                <h3 style="margin-top:0; font-size:18px;">Lock incentives?</h3>
    ...
    @if($showUnlockModal)
        <div style="position:fixed; inset:0; background:rgba(0,0,0,0.65); display:flex; align-items:center; justify-content:center; z-index:1000;">
            <div style="background:#111827; color:#e5e7eb; padding:20px; border-radius:12px; max-width:420px; width:90%; border:1px solid #374151;">
                <h3 style="margin-top:0; font-size:18px;">Unlock incentives?</h3>
