<div class="product-container">
    ...
    <div style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 12px;">
        <h2 class="page-heading" style="margin-bottom: 0;">Attendance</h2>
        <div style="display:flex; gap:10px; flex-wrap:wrap;">
            @can('attendance.create')
                <a href="{{ route('employee-attendance.create') }}" class="btn-primary" wire:navigate>Add Attendance</a>
            @endcan
        </div>
    </div>
        ...
        <table class="product-table hover-highlight">
            <thead>
                <tr>
                    ...
                    <th class="px-4 py-2 border dark:border-zinc-700">Employees</th>
            ...
            <tbody>
                @forelse ($records as $record)
                    <tr>
                        ...
                        <td class="px-4 py-2 border dark:border-zinc-700">{{ $record->attendance_date?->format('Y-m-d') }}</td>
                        <td class="px-4 py-2 border dark:border-zinc-700">{{ $record->lines_count }}</td>
                        <td class="px-4 py-2 border dark:border-zinc-700">
                            @if($record->is_locked)
                            ...
                            <span style="display:inline-flex; align-items:center; gap:8px; white-space:nowrap;">
                                @if(! $record->is_locked)
                                    @can('attendance.update')
                                        <a href="{{ route('employee-attendance.edit', $record->id) }}" class="action-link" wire:navigate>Edit</a>
                                    @endcan
                ...
                @empty
                    <tr>
                        <td colspan="5" class="px-4 py-2 border dark:border-zinc-700" style="text-align:center;">No attendance found.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
    ...
    @if($showLockModal)
        <div style="position:fixed; inset:0; background:rgba(0,0,0,0.65); display:flex; align-items:center; justify-content:center; z-index:1000;">
            <div style="background:#111827; color:#e5e7eb; padding:20px; border-radius:12px; max-width:420px; width:90%; border:1px solid #374151;">
                <h3 style="margin-top:0; font-size:18px;">Lock attendance?</h3>
    ...
    @if($showUnlockModal)
        <div style="position:fixed; inset:0; background:rgba(0,0,0,0.65); display:flex; align-items:center; justify-content:center; z-index:1000;">
            <div style="background:#111827; color:#e5e7eb; padding:20px; border-radius:12px; max-width:420px; width:90%; border:1px solid #374151;">
                <h3 style="margin-top:0; font-size:18px;">Unlock attendance?</h3>
